...
async def add_tags_to_book(
    book_id: str, tag_data: TagAddModel, session: AsyncSession = Depends(get_session)
) -> Book:
...
@tags_router.put(
    "/{tag_id}", response_model=TagModel, status_code= status.HTTP_200_OK,dependencies=[user_role_checker]
)
